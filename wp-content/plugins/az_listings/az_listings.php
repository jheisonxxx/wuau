...
function azl_get_default($args, $field) {
    if (isset($_GET['id']) && is_numeric($_GET['id'])) {
        ...
        if (isset($args['meta_key'])) {
            switch ($args['type']) {
                case 'file':
                    return wp_get_attachment_url(get_post_meta($_GET['id'], $args['meta_key'], true));
                    break;
                case 'file_list':
                    $ids = explode(',', get_post_meta($_GET['id'], $args['meta_key'], true));
                    $meta_value = array();
                    foreach ($ids as $id) {
                        if (is_numeric($id)) {
    ...
    if (isset($_GET['id']) && is_numeric($_GET['id'])) {
        ...
        if (!current_user_can('manage_options')) {
            if (!is_user_logged_in() || $post->post_author != $user_id) {
                return '<p class="cmb-error">' . __('You not have permission to edit this post.', 'azl') . '</p>';
            }
        }
    }
...
function azl_frontend_submission_handle($cmb, $post_data = array()) {
    ...
    if ($is_product) {
        ...
        update_post_meta($submission_id, '_visibility', 'visible');
                    ...
                    foreach ($forms as $form) {
                        if ($form['post_type'] == $post->post_type) {
                            if ($post->post_type == 'product') {
                                update_post_meta($_GET['id'], '_visibility', 'hidden');
                            } else {
                                $update['ID'] = $_GET['id'];
