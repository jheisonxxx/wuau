...
function custom_classes_register_meta() {
    ...
    register_meta(
            'post', '_custom_body_class', 'custom_classes_sanitize_meta', 'custom_classes_auth_post_meta'
    );

    /* Register custom post class meta. */
    register_meta(
            'post', '_custom_post_class', 'custom_classes_sanitize_meta', 'custom_classes_auth_post_meta'
    );
}
    ...
    if (is_singular()) {
        ...
        if (!empty($custom_class)) {
            ...
            foreach ($custom_classes as $cc) {
                $classes[] = sanitize_html_class($cc);
            }
        }
    }
...
function custom_classes_sanitize_meta($meta_value, $meta_key, $meta_type) {
    $sanitized = array();
    $classes = explode(' ', $meta_value);
    foreach ($classes as $cc) {
        $sanitized[] = sanitize_html_class($cc);
    }
