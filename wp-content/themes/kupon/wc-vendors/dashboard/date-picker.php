<form method="post">
    <div class="horizontal-list-2 p">
        <div>
            <label style="display:inline;" for="from"><?php esc_html_e('From:', 'wcvendors'); ?></label>
            <input class="date-pick" type="date" name="start_date" id="from"
                   value="<?php echo esc_attr(date('Y-m-d', $start_date)); ?>"/>
        </div>
        <div>
            <label style="display:inline;" for="to"><?php esc_html_e('To:', 'wcvendors'); ?></label>
    ...
    <p>
        <input type="submit" class="btn btn-inverse btn-small" style="float:none;"
               value="<?php esc_html_e('Show', 'wcvendors'); ?>"/>
    </p>
</form>